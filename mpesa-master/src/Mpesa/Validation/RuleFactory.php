...
use Kabangi\Mpesa\Validation\Rule\Callback as CallbackRule;
...
class RuleFactory
{
     ...
     * @param string $class
     *
     * @return \Kabangi\Mpesa\Validation\RuleFactory
     */
    public function register($name, $class, $errorMessage = '', $labeledErrorMessage = '')
    {
        if (is_subclass_of($class, '\Kabangi\Mpesa\Validation\Rule\AbstractRule')) {
     ...
     * @return \Kabangi\Mpesa\Validation\Rule\AbstractValidator
     */
    ...
    protected function construcRuleByNameAndOptions($name, $options)
    {
        if (is_callable($name)) {
            ...
            if (class_exists('\Kabangi\Mpesa\Validation\Rule\\' . $name, false)) {
                $name = '\Kabangi\Mpesa\Validation\Rule\\' . $name;
            }
            // at this point we should have a class that can be instanciated
            if (class_exists($name) && is_subclass_of($name, '\Kabangi\Mpesa\Validation\Rule\AbstractRule')) {
